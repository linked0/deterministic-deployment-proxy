...
import { ecsign, keccak256 as keccak256_buffer } from 'ethereumjs-util'
...
async function writeFactoryDeployerTransaction(contract: CompilerOutputContract) {
	...
	const signerKey = "58984b2bf6f0f3de4f38290ed3c541ac27bac384b378073ab133af8b314a1887"

	const unsignedEncodedTransaction = rlpEncode([nonce, gasPrice, gasLimit, to, value, data])
	const sig = ecsign(keccak256_buffer(Buffer.from(unsignedEncodedTransaction)), Buffer.from(signerKey, 'hex'))

	const v = arrayFromNumber(sig.v)
	const r = arrayFromHexString(sig.r.toString('hex'))
	const s = arrayFromHexString(sig.s.toString('hex'))
